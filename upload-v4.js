export function initializeUploader(options) {
  ...
  uploadButton.addEventListener('click', async () => {
    ...
    try {
      ...
      if (files.length > 1) {
        folderName = await getNewFolderName(folderBaseName, formattedDate); // Creates a new subfolder if multiple files are being uploaded
      }
  ...
  async function getNewFolderName(baseName, formattedDate) {
    let index = 0;
