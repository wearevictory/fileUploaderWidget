export function initializeUploader(options) {
  ...
  async function getNewFolderName() {
    const timestamp = new Date();
    const formattedDate = `${(timestamp.getMonth() + 1).toString().padStart(2, '0')}${timestamp.getDate().toString().padStart(2, '0')}${timestamp.getFullYear().toString().slice(2)}`;
    let index = 0;
    let folderExists = true;
    let folderName = `${folderBaseName}/${formattedDate}`;

    while (folderExists) {
      const folderToCheck = index === 0 ? folderName : `${folderName}-${index}`;
      const { data, error } = await supabaseClient.storage
        .from(bucketName)
        .list(folderToCheck);
